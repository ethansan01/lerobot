"""
AIRBOT Robot Client for LeRobot Async Inference

This client uses the LeRobot async inference API with AIRBOT hardware:
...
class AirbotClientConfig:
    """Configuration for AIRBOT async inference client."""

    # Policy configuration
    policy_type: str = "xvla"
    pretrained_name_or_path: str = "path/to/your/xvla/checkpoint"
...
class AirbotAsyncClient:
    ...
    def _build_lerobot_features(self) -> dict[str, dict]:
        ...
        During training, cameras were mapped as: top -> image, wrist -> image2
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        # The lerobot pipeline expects individual keys for state elements
        obs = {
            "image": frames["top"],
            "image2": frames["wrist"],
    ...
    def control_loop(self, verbose: bool = False):
        ...
        while self.running:
            ...
            if self._ready_to_send_observation():
                ...
                if raw_obs["image"] is None or raw_obs["image2"] is None:
...
def main():
    """Main entry point for AIRBOT async inference client."""
    import argparse

    parser = argparse.ArgumentParser(description="AIRBOT Async Inference Client")
    parser.add_argument("--server", type=str, default="127.0.0.1:8080", help="Policy server address")
    parser.add_argument("--policy-type", type=str, default="xvla", help="Policy type (e.g., xvla, pi0, act)")
    ...
    print("\n" + "=" * 50)
    print(" AIRBOT Async Inference Client")
    print("=" * 50)
