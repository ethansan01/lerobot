"""
AIRBOT + Revo2 Robot Client for LeRobot Async Inference

This client mirrors the structure of examples/validation/airbot_robot_client.py
but replaces the single gripper channel with a 6-DoF Revo2 dexterous hand.

Observation/state layout:
    [arm_qpos(6), hand_qpos(6)]

Action layout:
    [arm_cmd(6), hand_cmd(6)]
...
2. Run this client:
   python examples/validation/airbot_robot_revo2_client.py \
       --pretrained /path/to/checkpoint \
       --arm-port 50001 \
       --hand-port /dev/ttyUSB1
"""
...
class DualOrbbecStreamer:
    """Manages the top and wrist Orbbec cameras."""
...
class AirBotArm:
    ...
    def act(self, action: np.ndarray):
        if not self.robot:
            print("[ROBOT] Warning: arm not connected!")
            return
        print(f"[ROBOT] Arm cmd: {np.array2string(action[:ARM_DOF], precision=4, suppress_small=True)}")
    ...
    def move_home(self):
        ...
        home = [-0.17299915850162506, -0.9233615398406982, 0.4060807228088379, 1.2960631847381592, 1.2266346216201782, 0.07877469807863235]
...
class Revo2Hand:
    ...
    def _start_loop(self):
        ...
        self.loop_thread = threading.Thread(target=runner, daemon=True, name="revo2-loop")
    ...
    def connect(self):
        print(f"[HAND] Connecting to Revo2 hand on port {self.port}...")
        self._start_loop()
    ...
    def get_state(self) -> np.ndarray:
        ...
        try:
            ...
            print("[HAND] Timed out reading Revo2 joint positions")
        except Exception as exc:
            print(f"[HAND] Failed to read Revo2 joint positions: {exc}")
        return np.zeros(HAND_DOF, dtype=np.float32)

    def act(self, action: np.ndarray):
        if not self.connected:
            print("[HAND] Warning: hand not connected!")
            return

        try:
            robot_positions = self._map_to_robot(action[:HAND_DOF])
            # print(f"[HAND] Hand cmd model: {np.array2string(action[:HAND_DOF], precision=4, suppress_small=True)}")
            # print(f"[HAND] Hand cmd robot: {robot_positions}")
            self._run(self._set_positions_async(robot_positions))
        except concurrent.futures.TimeoutError:
            print("[HAND] Timed out sending Revo2 joint positions")
        except Exception as exc:
            print(f"[HAND] Failed to send Revo2 joint positions: {exc}")


class AirBotRevo2Robot:
    """Combined AIRBOT arm + Revo2 hand interface."""

    def __init__(self, arm_port: str, hand_port: str):
        self.arm = AirBotArm(port=arm_port)
        self.hand = Revo2Hand(port=hand_port)
    ...
    def get_state(self) -> np.ndarray:
        ...
        return np.concatenate([arm_state, hand_state]).astype(np.float32)

    def act(self, action: np.ndarray):
        arm_action = np.asarray(action[:ARM_DOF], dtype=np.float32)
        hand_action = np.asarray(action[ARM_DOF : ARM_DOF + HAND_DOF], dtype=np.float32)

        try:
            rr.log("robot/arm_cmd", rr.BarChart(arm_action))
            rr.log("robot/hand_cmd", rr.BarChart(hand_action))
        except Exception:
            pass

        self.arm.act(arm_action)
        self.hand.act(hand_action)


@dataclass
class AirbotRevo2ClientConfig:
    """Configuration for AIRBOT + Revo2 async inference client."""
...
class AirbotRevo2AsyncClient:
    """AIRBOT + Revo2 client for the LeRobot async inference server."""

    prefix = "airbot_revo2_client"
    logger = get_logger(prefix)

    def __init__(self, config: AirbotRevo2ClientConfig):
        self.config = config
        ...
        self.robot = AirBotRevo2Robot(arm_port=config.arm_port, hand_port=config.hand_port)
    ...
    def _build_lerobot_features(self) -> dict[str, dict]:
        return {
            ...
            "observation.state": {
                "dtype": "float32",
                "shape": (ARM_DOF + HAND_DOF,),
                "names": [f"state_{i}" for i in range(ARM_DOF + HAND_DOF)],
            },
        }
    ...
    def control_loop(self, verbose: bool = False):
        ...
        while self.running:
            ...
            if self._ready_to_send_observation():
                ...
                if raw_obs["top"] is None or raw_obs["wrist_left"] is None:
                    time.sleep(0.01)
                    continue
...
def main():
    ...
    parser = argparse.ArgumentParser(description="AIRBOT + Revo2 Async Inference Client")
    ...
    parser.add_argument("--hand-port", type=str, default="/dev/ttyUSB0", help="Revo2 hand port")
    ...
    rr.init("airbot_revo2_async_client", spawn=True)

    config = AirbotRevo2ClientConfig(
    ...
    client = AirbotRevo2AsyncClient(config)

    print("\n" + "=" * 50)
    print(" AIRBOT + REVO2 Async Inference Client")
    print("=" * 50)
    print(f" Server: {config.server_address}")
    print(f" Policy: {config.policy_type} ({config.pretrained_name_or_path})")
    print(f" Arm port: {config.arm_port}")
    print(f" Hand port: {config.hand_port}")
    print(f" State/action layout: arm(6) + hand(6)")
    print("=" * 50)
